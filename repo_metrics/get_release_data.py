...
import pandas as pd
import numpy as np
def get_repo_releases(url, token):
...
if __name__ == "__main__":
    ...
    for i in range(len(idxs)-1):
        info = dataset_github["url"].iloc[idxs[i]:idxs[i+1]].apply(get_repo_releases,
                                                                       token=token)
        info.to_csv(f"../../rs_usage/info_repos/release_{idxs[i+1]}.csv", escapechar="~")
